...
$target = "upload/Contest1/";  //folder where it goes + ADD FOLDER with NAME of applicant/email address
...
$response_ar = $turboApiClient->sendEmail($email_ar);
	...
	if (($response['message'] == 'OK') && ($response_ar['message'] == 'OK')){ // Si le mail a 彋?envoy?
		...
		if($ok==1 and $prob==0){
			header('Location: entryForm.html#paypal');
		}
		else{
			echo "<br><br>".$prob." problem(s) detected.";
			echo "<br><a href=entryForm.html#form>back</a>"; // Avec un lien de retour vers l'accueil du site
		}
		...
		if($ok==1 and $prob==0){
			...
			echo "<br>You can proceed the <a href=entryForm.html#paypal>submission</a>";
		}
		else{
			echo "<br><br>".$prob." problem(s) detected.";
			echo "The upload was unsuccessful and an issue occured while sending the confirmation email.\n";
			echo "<br>Please <a href=entryForm.html#form>try again</a>"; // Avec un lien de retour vers l'accueil du site
		}
